...
void JPSPathFinder::JPSGraphSearch(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
    ...
    startPtr->gScore = 0;
    startPtr->fScore = getHeu2(startPtr, endPtr);
    ...
    while (!openSet.empty())
    {
        ...
        if (currentPtr->index == goalIdx)
        {
            ...
            ROS_WARN("[Better_A*_JPS_Black] succeed ! Time consumed : %f ms, path cost : %f m", (time_2 - time_1).toSec() * 1000.0, currentPtr->gScore * resolution);
            return;
        }
        ...
        for (int i = 0; i < (int)neighborPtrSets.size(); i++)
        {
            ...
            { 
                ...
                neighborPtr->fScore = neighborPtr->gScore + getHeu2(neighborPtr, endPtr);
